...
class ReconstructionLoss(nn.Module):
    ...
    def forward(
        ...
        total = j_E + self.lambda_S * j_S
        return total, {'edge_loss': j_E.item(), 'node_loss': j_S.item(), 'recon_loss': total.item()}
...
class SparseCodingLoss(nn.Module):
    ...
    def forward(
        ...
        total = recon_error + self.lambda_SC * l1_penalty
        return total, {'sc_recon_error': recon_error.item(), 'sc_l1_penalty': l1_penalty.item(), 'sc_loss': total.item()}
    ...
    def forward(
        ...
        if 'features' in outputs and 'sc_reconstruction' in outputs:
            sc_loss, sc_metrics = self.sc_loss(
                outputs['features'], outputs['sc_reconstruction'], outputs['codes'],
            )
