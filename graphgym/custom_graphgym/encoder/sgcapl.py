...
class SGCAPL(nn.Module):
    def __init__(
        ...
        self.lstm_cells = nn.ModuleList([
            PeepholeLSTMCell(hidden_dim, hidden_dim) for _ in range(n_layers)
        ])
    ...
    def forward(
        ...
        c_states: List[torch.Tensor] = None,
    ) -> Tuple[torch.Tensor, List, List]:
        N = nodes.shape[0]
        device = nodes.device
        hidden_size = self.lstm_cells[0].hidden_size
        if h_states is None:
            h_states = [torch.zeros(N, hidden_size, device=device) for _ in range(self.n_layers)]
        if c_states is None:
            c_states = [torch.zeros(N, hidden_size, device=device) for _ in range(self.n_layers)]

        x = self.sgc(nodes, adjacency)
        x = self.attention(x, adjacency)

        new_h_states, new_c_states = [], []
        ...
        return self.output_proj(x), new_h_states, new_c_states
