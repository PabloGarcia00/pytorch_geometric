...
class MultiHeadAttention(nn.Module):
    def __init__(self, in_features: int, out_features: int, n_heads: int = 3):
        ...
        self.attention_kernels = nn.ModuleList([
            nn.Linear(2 * out_features, 1) for _ in range(n_heads)
        ])

    def forward(self, x: torch.Tensor, adjacency: torch.Tensor) -> torch.Tensor:
        N = x.shape[0]
        device = x.device
        mask = (adjacency > 0).float() + torch.eye(N, device=device)
        outputs = []
        for r in range(self.n_heads):
            h = self.W[r](x)
            h_i = h.unsqueeze(1).expand(N, N, self.out_features)
            h_j = h.unsqueeze(0).expand(N, N, self.out_features)
            e = self.attention_kernels[r](torch.cat([h_i, h_j], dim=-1)).squeeze(-1)
            e = F.leaky_relu(e, negative_slope=0.2)
            e = e * mask + (1 - mask) * (-1e9)
            alpha = F.softmax(e, dim=1)
            outputs.append(torch.matmul(alpha, h))
        return F.relu(torch.stack(outputs).mean(dim=0))
