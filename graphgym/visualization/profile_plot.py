...
def _load_model(latest_ckpt: Path, device: torch.device):
    ...
    if model_keys[0].startswith("model.") and not ckpt_keys[0].startswith(
        "model."
    ):
        state_dict = {f"model.{k}": v for k, v in state_dict.items()}
    elif ckpt_keys[0].startswith("model.") and not model_keys[0].startswith(
        "model."
    ):
        state_dict = {
...
def active_diverse_selection(
    dataset, node_idx, candidates, num_steps, k=2, threshold=1e-5
):
    ...
    print(
        f"Searching for {k} active/diverse windows among {len(candidates)} candidates..."
    )
    ...
    for idx in candidates:
        ...
        l_prof = (
            full_ds.load_scaled[
                idx + seq_len : idx + seq_len + num_steps, node_idx
            ]
            .cpu()
            .numpy()
        )
    ...
    if not active_cand:
        print(
            f"Warning: No candidates met threshold {threshold}. Returning first {k} available."
        )
        return candidates[:k]
...
def collect_results(
    model, dataset, node_idx, start_idx, num_steps, device, p1, p2, denorm_fn
):
    ...
    # Denormalise then convert W -> kW
    for key, v in res.items():
        t = torch.tensor(v, dtype=torch.float)
        res[key] = denorm_fn(t, p1, p2).numpy() / 1000.0
    ...
    if hasattr(full_ds, "timestamps") and full_ds.timestamps is not None:
        ...
        res["title_end"] = str(
            full_ds.timestamps[start_idx + seq_len + num_steps - 1]
        )[:10]
    else:
...
def plot_multi(results_list, out_path, is_weekly):
    ...
    for i, res in enumerate(results_list):
        ...
        if i == 0:
            ...
            sorted_handles = [
                label_to_handle[l] for l in order if l in label_to_handle
            ]
    ...
    print(
        f"{'Weekly (1x2)' if is_weekly else 'Daily (2x1)'} Plot saved to: {out_path}"
    )
...
if __name__ == "__main__":
    ...
    for i in indices:
        ...
        w = batch.weather[0]
        is_midnight = (
            torch.abs(w[_W_COS_TOD] - 1.0) < 0.01
            and torch.abs(w[_W_SIN_TOD]) < 0.01
        )
        is_monday = (
            torch.abs(w[_W_COS_DOW] - 1.0) < 0.01
            and torch.abs(w[_W_SIN_DOW]) < 0.01
        )
    ...
    selected_idx = active_diverse_selection(
        dataset, args.node, candidates, num_steps, k=2
    )
    results = [
        collect_results(
            model, dataset, args.node, idx, num_steps, device, p1, p2, denorm_fn
        )
