...
def grouped_bar(scores, x_label="Dataset", y_label="MAE", legend_title=None):
    ...
    for i, bar_name in enumerate(bars_per_group):
        ...
        for j, group_name in enumerate(groups):
            ...
            pos = j * (n_bars * cfg["bar_width"] + cfg["group_gap"]) + (
                i * cfg["bar_width"]
            )
    ...
    tick_positions = [
        j * (n_bars * cfg["bar_width"] + cfg["group_gap"])
        + (cfg["bar_width"] * (n_bars - 1) / 2)
...
def load_metrics(grid_dir, metric_name):
    ...
    for subdir in p.iterdir():
        ...
        config = parse_config(subdir.name)

        # Extract what you need
        # w_val = config.get("w", None)
        # mp_val = config.get("mp", None)

        weather = "Unknown"
        # if w_val is not None:
        #     weather = "Weather" if len(w_val) > 2 else "No Weather"

        # mp = f"MP={mp_val}" if mp_val is not None else "Unknown"
        ...
        if scores:
            ...
            results[key] = scores  # store full config if you want flexibility
    return results


def to_df(metrics, name):
    import pandas as pd

    df = pd.DataFrame(
        [{**dict(params), name: val[0]} for params, val in metrics.items()]
    )
    return df.sort_values(by=name)


def to_dict(df, val, keys=[]):
    scores = defaultdict(list)

    for _, row in df.iterrows():
        key = (row[k] for k in keys)
        scores[key].append(row["mae_pv"])
    return scores


# cdir = "./results/earne_exp2_grid_exp2/"
# pv = to_df(load_metrics(cdir, "earne_mae_pv"), "mae_pv")
# load = to_df(load_metrics(cdir, "earne_mae_load"), "mae_load")
# loss = to_df(load_metrics(cdir, "loss"), "MSE")
# loss = to_dict(loss[loss["norm_mode"] == "ihs"], "MSE", ["graph_mode", ""])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dir", type=str, required=True, help="Results grid directory"
    )
    parser.add_argument(
        "--metric", type=str, default="earne_mae_load", help="Metric to plot"
    )
    parser.add_argument(
        "--out", type=str, default="plot.pdf", help="Output filename"
    )
    ...
    if not data:
        ...
        for key, vals in data.items():
            print(
                f"{key}: {len(vals)} seed(s) found. Mean: {np.mean(vals):.4f}"
            )
        print("----------------\n")

        label = args.metric.replace("earne_mae_", "").upper()
        fig = grouped_bar(
            data, x_label="Data Configuration", y_label=f"{label} MAE"
        )
