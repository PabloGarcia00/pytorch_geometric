...
class GraphBuilder:
    def __init__(self, lambda_threshold: float = 0.25, cache_size: int = 100):
        self.lambda_threshold = lambda_threshold

    def build_graph(self, net_demand: torch.Tensor, window: int = 10, lambda_threshold: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        N, T = net_demand.shape
        device = net_demand.device
        
        # Use provided learnable threshold or the static one
        ...
        # Differentiable thresholding using Sigmoid relaxation
        if lambda_threshold is not None:
            # High k makes it closer to a step function
            k = 100.0
...
class LaplacianCalculator:
    ...
    def compute_normalized_laplacian(adjacency: torch.Tensor) -> torch.Tensor:
        ...
        laplacian = torch.eye(N, device=adjacency.device) - \
                   torch.matmul(torch.matmul(degree_sqrt_inv, adjacency), degree_sqrt_inv)
        return laplacian
